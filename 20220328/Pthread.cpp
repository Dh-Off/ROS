...
/* 
 쓰레드 생성을 위해서 사용한다.
 1번째 아규먼트인 thread는 쓰레드가 성공적으로 생성되었을때 생성된 쓰레드를 식별하기 위해서 사용되는 쓰레드 식별자
 2번째 아규먼트인 attr은 쓰레드 특성을 지정하기 위해 사용, 기본 쓰레드 특성을 이용하고자 할경우에 NULL 을 사용
...
/* 
 첫번째 아규먼트 th는 기다릴(join)할 쓰레드 식별자이며, 두번째 아규먼트 thread_return은 쓰레드의 리턴(return) 값이다. 
...
/* 
 ...
 이 함수는 식별번호th인 쓰레드를 detach 시키는데, detach 되었을경우 해당(detach 된) 쓰레드가 종료될경우 pthread_join 을 호출하지 않더라도 즉시 모든 자원이 해제(free) 된다.
 */

 void pthread_exit(void *retval);

 /* 
  현재 실행중인 쓰레드를 종료시키고자 할때 사용
 ...
 /* 
  ...
  pthread_exit(3)가 호출되어서 쓰레드가 종료될때 pthread_cleanup_push에 의해서 인스톨된 함수가 호출된다.
  routine이 쓰레드가 종료될때 호출되는 함수, arg는 아규먼트이다.
  cleanup handlers 는 주로 자원을 되돌려주거나, mutex 잠금등의 해제를 위한 용도로 사용사용된다.
  만약 mutex 영역에서 pthread_exit 가 호출되어 버릴경우 다른쓰레드에서 영원히 block 될수 있기 때문이다.
  malloc 으로 할당받은 메모리, 열린 파일지정자를 닫기 위해서도 사용한다.
  */
 ...
 /*
  ...
  만약 이미 다른 쓰레드에서 mutex lock 를 얻어서 사용하고 있다면 다른 쓰레드에서 mutex lock(뮤텍스 잠금) 을 해제할때까지 블럭 된다.
 ...
 /*
  ...
  위 함수를 쓸때는 interrupted 상황에 대한 처리를 해주어야 한다.
  */
 ...
 /*
  쓰레드의 상태를 PTHREAD_CREATE_JOINABLE 혹은 PTHREAD_CREATE_DETACHED 상태로 변경시키기 위해서 사용하면 된다.
  */
